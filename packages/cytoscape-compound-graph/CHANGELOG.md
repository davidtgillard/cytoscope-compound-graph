...
- Root (parentless) leaves stay natively grabbable in `CompoundGraphScene`: only parented leaves are ungrabified for detached child-drag. `attachParentDragHandlers` + `moveRootLeaf` sync and persist root-leaf moves the same way container drag does, so the documented two-handler setup is enough.
